...
class PPCP_Paypal_Checkout_For_Woocommerce_Response {
    ...
    public function __construct() {
        ...
        $this->is_sandbox = 'yes' === $this->settings->get('sandbox', 'no');
    }
    ...
    public function parse_response($paypal_api_response, $url, $request, $action_name) {
        try {
            ...
            do_action('ppcp_request_respose_data', $request, $response, $action_name);
            $this->ppcp_write_log($url, $request, $paypal_api_response, $action_name);
            return $response;
        } catch (Exception $ex) {
    ...
    public function ppcp_write_log($url, $request, $response, $action_name = 'Exception') {
        global $wp_version;
        ...
        if (strpos($action_name, 'webhook') !== false) {
            ...
            $this->api_log->webhook_log('PFW Version: ' . WPG_PLUGIN_VERSION);
            ...
            $this->api_log->log('PFW Version: ' . WPG_PLUGIN_VERSION);

...
class PPCP_Paypal_Checkout_For_Woocommerce_Subscriptions_CC extends PPCP_Paypal_Checkout_For_Woocommerce_Gateway_CC {

    public function __construct() {
        parent::__construct();
        if (class_exists('WC_Subscriptions_Order')) {
            if (!has_action('woocommerce_scheduled_subscription_payment_wpg_paypal_checkout_cc', array($this, 'scheduled_subscription_payment'))) {
                add_action('woocommerce_scheduled_subscription_payment_wpg_paypal_checkout_cc', array($this, 'scheduled_subscription_payment'), 10, 2);
            }
            ...
            add_action('woocommerce_subscription_failing_payment_method_updated_wpg_paypal_checkout_cc', array($this, 'update_failing_payment_method'), 10, 2);
    ...
    public function scheduled_subscription_payment($amount_to_charge, $renewal_order) {
        ...
        try {
            ...
            if (function_exists('wc_get_logger')) {
                wc_get_logger()->error('CC subscription renewal error: ' . $e->getMessage(), array('source' => 'wpg_paypal_checkout'));
            }
            $renewal_order = wc_get_order($renewal_order->get_id());
    ...
    public function free_signup_with_token_payment_tokenization($order_id) {
        ...
        $token_value = isset($_POST['wc-wpg_paypal_checkout_cc-payment-token']) ? sanitize_text_field(wp_unslash($_POST['wc-wpg_paypal_checkout_cc-payment-token'])) : '';
